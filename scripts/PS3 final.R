rm(list=ls())
setwd("C:/Users/MIGOMLOZ/Desktop/PS3/")

# importar librerias
## Llamar pacman (contiene la funcion p_load)
require(pacman)
#install.packages("car")

## Llama/instala-llama las librerias listadas
...
# Separar bogota y medellin

house_bog <- house_union %>% filter(l3 == 'Bogot? D.C')
house_med <- house_union %>% filter(l3 == 'Medell?n')
...
house_test_bog <- house_test %>% filter(l3 == 'Bogot? D.C')
house_test_med <- house_test %>% filter(l3 == 'Medell?n')

house_train_bog <- house_train %>% filter(l3 == 'Bogot? D.C')
house_train_med <- house_train %>% filter(l3 == 'Medell?n')
...
#definir poligono de chapinero
chapinero <- getbb(place_name = "UPZ Chapinero, Bogota", 
                   featuretype = "boundary:administrative", 
                   format_out = "sf_polygon") %>% .$multipolygon

#definir poligono de Medellin
medellin <- getbb(place_name = "Medell?n",
                  featuretype = "boundary:administrative",
...
#chapinero
# Pegar precio de nuevo

#house_train_bog2 = st_join(house_train_bog,house_train_price, left = TRUE,largest = FALSE)
#house_train_med = st_join(house_train_med,house_train_price, left = TRUE)

chapi_train <- house_train_bog[chapinero,]
chapi_test <- house_test_bog[chapinero,]
chapi_mnz <- mnz_bog[chapinero,]

leaflet() %>% addTiles() %>% addPolygons (data = chapinero, color="red")  
  %>% addCircles (data = chapi_test, color="green") 
  %>% addCircles (data = chapi_train, color="purple")
...
# Medir la minima distancia de los apartamentos a las estaciones de transporte
...
saveRDS(chapi_train, "chapi_train.rds")
saveRDS(chapi_test, "chapi_test.rds")
saveRDS(house_train_med, "house_train_med.rds")
saveRDS(house_test_med, "house_test_med.rds")
...
chapi_1 <- chapi_train$price ~ chapi_train$dist_east+chapi_train$dist_chapi_bus + chapi_train$bedrooms + chapi_train$tiene_terraza + chapi_train$tiene_garaje+ chapi_train$surface2
med_1 <- house_train_med$price ~ house_train_med$dist_golf + house_train_med$dist_med_bus + house_train_med$bedrooms + house_train_med$tiene_terraza + house_train_med$tiene_garaje+ house_train_med$surface2
...
chapi_train$surface22 <- chapi_train$surface2*chapi_train$surface2
...
house_train_med$surface22 <- house_train_med$surface2*house_train_med$surface2
...
# Random Forest - se utiliz? rpart function para lidiar con los NAs
...
chapi_matriz_xg <- chapi_train[,c(1,4,6,7,8,12:17)]

chapi_matriz_xg$identificador <- 1:13451
...
chapi_matriz1_xg <- chapi_matriz_xg[,c(2,3,6,7,8,12,14,16)]
...
chapi_matriz2_xg <- chapi_matriz_xg[,c(2,3,6,9,10,11,12,14,16)]
...
stargazer(lm_med1,type = "text",title = "Medell?n OLS", out = "med1.doc")

lm_med2 <- lm(med_2, data=house_train_med)
pred_ols_med2 <- predict(lm_med2)
stargazer(lm_med2,type = "text",title = "Medell?n OLS", out = "med2.doc")
...
med_matriz_xg <- house_train_med[,c(1,4,6,7,8,12:17)]
...
med_matriz1_xg <- med_matriz_xg[,c(2,3,6,7,8,12,14,16)]
med_matriz1_xg <- as.matrix(med_matriz1_xg)

med_matriz2_xg <- med_matriz_xg[,c(2,3,6,9,10,11,12,14,16)]
med_matriz2_xg <- as.matrix(med_matriz2_xg)

xgb_pob1 <- xgboost(data = med_matriz1_xg[,c(1:8)], label = house_train_med$price, max.depth = 6, eta = 0.5, nrounds = 100, objective = "reg:squarederror")

xgb_pob2 <- xgboost(data = med_matriz2_xg[,c(1:8)], label = house_train_med$price, max.depth = 6, eta = 0.5, nrounds = 100, objective = "reg:squarederror")
...
base1_capi <-chapi_matriz_xg[,c(2,3,6,7,8,14,16)]
base1_capi <- as.data.frame(base1_capi)

base1_capi <- base1_capi %>% drop_na()

outcome_chapi = base1_capi$price
base1_capi <- base1_capi %>% select(-price)

# Exportar base 1 chapi + outcome
saveRDS(base1_capi, "base1_capi.rds")
saveRDS(outcome_chapi, "outcome_chapi.rds")
...
fitprice_chapi <- SuperLearner(Y=outcome_chapi, X= base1_capi,
...
fitprice_chapi <- SuperLearner(Y=outcome_chapi, X= base1_capi, newX = NULL,
                               method = "method.NNLS", SL.library = c("SL.lm","SL.rpart","SL.xgboost"))

# Chapinero variables cuadr?ticas
base2_capi <-chapi_matriz_xg[,c(2,3,9,10,11,14,16)]
base2_capi <- as.data.frame(base2_capi)

base2_capi <- base2_capi %>% drop_na()
base2_capi <- base2_capi %>% select(-price)

# Exportar base
saveRDS(base2_capi, "base2_capi.rds")
...
## Medell?n variables lineales
base1_med <-med_matriz_xg[,c(2,3,6,7,8,14,16)]
base1_med <- as.data.frame(base1_med)
base1_med <- base1_med %>% drop_na()
outcome_med = base1_med$price
base1_med <- base1_med %>% select(-price)

saveRDS(base1_med, "base1_med.rds")
saveRDS(outcome_med, "outcome_med.rds")

fitprice_med <- SuperLearner(Y=outcome_med, X= base1_med, newX = NULL,
                             method = "method.NNLS", SL.library = c("SL.lm","SL.rpart","SL.xgboost"))
fitprice_med

#Medell?n variables cuadr?ticas

base2_med <-med_matriz_xg[,c(2,3,9,10,11,14,16)]
base2_med <- as.data.frame(base2_med)

base2_med <- base2_med %>% drop_na()
base2_med <- base2_med %>% select(-price)

saveRDS(base2_med, "base2_med.rds")
...
# Predict
y_yat_chapi <- predict(fitprice2_chapi,new_data = chapi_test)
write.csv(y_yat_chapi ,"y_yat_chapi")
y_yat_med <- predict(fitprice2_med,new_data = house_test_med)
write.csv(y_yat_med ,"y_yat_med")
